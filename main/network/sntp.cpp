...
void Ntp::obtainTime() {
	...
	for (uint8_t retry = 0; esp_sntp_get_sync_status() == SNTP_SYNC_STATUS_RESET && retry < maxRetries; ++retry) {
...
Ntp::Ntp(Mcp7940n *rtc) {
	...
	for (uint8_t i = 0; i < serverCount; ++i) {
		...
		esp_sntp_setservername(i, this->servers[i].c_str());
		ESP_LOGI(TAG, "Server #%u: %s", i, this->servers[i].c_str());
	}
	esp_sntp_set_time_sync_notification_cb(Ntp::notifySyncronization);
	...
	if (esp_sntp_get_sync_mode() == SNTP_SYNC_MODE_SMOOTH) {
		struct timeval outdelta;
		while (esp_sntp_get_sync_status() == SNTP_SYNC_STATUS_IN_PROGRESS) {
